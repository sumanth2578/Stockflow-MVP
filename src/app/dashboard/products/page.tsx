    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-slate-900 px-4 py-2 rounded text-sm font-bold text-white transition-colors hover:bg-slate-800"
        >
          <Plus size={16} />
          Create Product
        </button>
      ...
      <div className="bg-white p-4 rounded border border-slate-200 shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded border border-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-900/5 focus:border-slate-900 text-sm transition-all text-slate-900"
            value={search}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Product Info</th>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">SKU</th>
                  <th className="px-6 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Stock</th>
                ...
                {filteredProducts.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-slate-500 text-xs truncate max-w-[200px]">{p.description || "N/A"}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-mono text-[10px] font-bold bg-slate-100 px-1.5 py-0.5 rounded text-slate-500 uppercase">{p.sku}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className={`font-bold ${p.quantity <= (p.lowStockThreshold ?? 5) ? 'text-red-600' : 'text-slate-900'}`}>
                        {p.quantity}
                      </span>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="text-xs font-bold uppercase tracking-widest text-slate-900">
                {editingProduct ? "Update Entry" : "Create New Entry"}
              </h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <input
                    type="text"
                    required
                    className="w-full px-3 py-2 rounded border border-slate-200 focus:outline-none focus:border-slate-900 text-sm text-slate-900 placeholder-slate-300"
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <input
                    type="text"
                    required
                    className="w-full px-3 py-2 rounded border border-slate-200 focus:outline-none focus:border-slate-900 text-sm font-mono text-slate-900 placeholder-slate-300"
                    placeholder="TSH-001"
                    value={formData.sku}
                ...
                <div className="col-span-2 md:col-span-1">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Initial Units</label>
                  <input
                    type="number"
                    required
                ...
                <div className="col-span-2 md:col-span-1">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Cost ($)</label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div className="col-span-2 md:col-span-1">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Sale ($)</label>
                  <input
                    type="number"
                    step="0.01"
                    className="w-full px-3 py-2 rounded border border-slate-200 focus:outline-none focus:border-slate-900 text-sm text-slate-900"
                ...
                <div className="col-span-2">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Product Notes</label>
                  <textarea
              ...
              <div className="pt-6 flex items-center justify-end gap-3 flex-col sm:flex-row">
                 <button
                  type="submit"
                  className="w-full sm:w-auto px-10 py-2.5 bg-slate-900 text-white text-xs font-bold uppercase tracking-widest rounded hover:bg-slate-800 transition-colors order-1 sm:order-2"
                >
                ...
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="w-full sm:w-auto px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors order-2 sm:order-1"
                >
                  Discard
                </button>
              </div>
